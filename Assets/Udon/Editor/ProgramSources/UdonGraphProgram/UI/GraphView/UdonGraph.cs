    ...
    public class UdonGraph : UnityGraph.GraphView
    {
        ...
        private GraphViewChange OnViewChanged(GraphViewChange changes)
        {
            ...
            if (!_reloading && changes.elementsToRemove != null && changes.elementsToRemove.Count > 0)
            {
                ...
                foreach (var element in changes.elementsToRemove)
                {
                    ...
                    var graphElement = newElementsData?.Find(e => e != null && e.uid != null && e.uid.CompareTo(element.persistenceKey) == 0);
        ...
        public void Reload()
        {
            ...
            for (int i = graphData.nodes.Count - 1; i >= 0; i--)
            {
                ...
                // Temp Migration of Midi Nodes
                // Todo: remove this before release, just fixes issue in closed beta
                if (nodeData.fullName.StartsWithCached("Event_OnNote"))
                {
                    string ogName = nodeData.fullName;
                    nodeData.fullName = nodeData.fullName.Replace("OnNote", "MidiNote");
                    Debug.LogWarning($"Converted {ogName} to {nodeData.fullName}");
                }
                else if (nodeData.fullName.StartsWithCached("Event_OnControlChange"))
                {
                    nodeData.fullName = "Event_MidiControlChange";
                    Debug.LogWarning($"Converted Event_OnControlChange to {nodeData.fullName}");
                }
                ...
                if (nodeData.fullName.StartsWithCached("Variable_"))
                {
                    ...
                    if (definition != null && !IsReservedName((string)nodeData.nodeValues[1].Deserialize()))
                    {
