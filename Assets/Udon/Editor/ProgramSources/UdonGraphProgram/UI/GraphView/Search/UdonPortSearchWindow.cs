...
namespace VRC.Udon.Editor.ProgramSources.UdonGraphProgram.UI.GraphView
{
    ...
    public class UdonPortSearchWindow : UdonSearchWindowBase
    {
        ...
        {
            public string uid;
            public UdonGraph.VariableNodeType variableType;

            public VariableInfo(string uid, UdonGraph.VariableNodeType variableType)
            {
                this.uid = uid;
        ...
        override public List<SearchTreeEntry> CreateSearchTree(SearchWindowContext context)
        {
            ...
            foreach (var item in variables)
            {
                ...
                if(variableSimpleName == typeToSearchSimple)
                {
                    ...
                    {
                        level = 1,
                        userData = new VariableInfo(item.uid, direction == Direction.Output ? UdonGraph.VariableNodeType.Getter : UdonGraph.VariableNodeType.Setter),
                    });
                }
            }
        ...
        override public bool OnSelectEntry(SearchTreeEntry entry, SearchWindowContext context)
        {
            ...
            else if(entry.userData is VariableInfo)
            {
                ...
                UdonNode udonNode = _graphView.MakeVariableNode(data.uid, position, data.variableType);
